...
- **Diff viewer v0 shipped** — `ui/diff.scrml` (~85 LOC) shows raw working-copy diff (`engine.diff()`). Route `/_scrml/__ri_route_loadWorkingDiff_2`. Nav link added to all four pages (Status/History/Bookmarks/Diff). End-to-end verified via curl: CSRF bootstrap+retry, 200 response with engine output. v1 open: per-change selection using `jj diff -r <change>` + `route.query` for change id.
