...
// friendlyError — map common jj stderr patterns to friendlier messages.
    ...
    export function friendlyError(stderr) {
        ...
        // The `no conflicts` guard matters: jj's `resolve --list` reports
        // "No conflicts found at this revision" on a CLEAN tree, which matches
        // /conflict/i and would otherwise be rewritten into its own opposite
        // ("Merge conflict detected"). Found S20 wiring the AST-merge layer.
        if (/conflict/i.test(raw) && !/resolved/i.test(raw) && !/no conflicts?\b/i.test(raw)) {
