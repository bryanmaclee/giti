...
// normalizeRelPath — repo-relative path to forward-slash form for
    ...
    //
    // DF-6/DF-7: the original JS imported `sep` from node:path. scrml
    // requires either a `vendor:` shim file or a `scrml:`-stdlib import,
    // so we side-step here by normalizing any backslash to forward-slash
    // directly. (jj/giti paths are always forward-slash; the JS path
    // was defensive against native-separator inputs that giti itself
