...
**Open (UI-blocking):**
- [ ][ ] **GITI-009** — scrmlTS forwards relative imports verbatim; source-relative paths don't resolve from compiled-output location. Confirmed at runtime S7: `bun run src/cli.js serve` throws `Cannot find module './repro-06-relative-imports-helper.js' from 'dist/ui/repro-06-relative-imports.server.js'`. Workaround applied in `ui/status.scrml` with `../../src/...` prefix. Repro: `ui/repros/repro-06-relative-imports.scrml` + helper `.js`. Sent to scrmlTS 2026-04-22.
- [ ][ ] **GITI-011** — scrml CSS parser has no at-rule handling. `@import`, `@media`, `@keyframes`, `@font-face`, `@supports`, `@page` all fall through the property-declaration path and emit `ident: ; rest-of-line ...`, hoisting any nested body out of the at-rule wrapper. Same failure shape as pre-fix GITI-007. Confirmed against scrmlTS `8691f75` S7. Blocks responsive design + animations in every scrml UI. Workaround for `@import` (giti S7): HTML link injection. No workaround for `@media` / `@keyframes`. Repro: `ui/repros/repro-07-css-at-rules.scrml`. Sent to scrmlTS 2026-04-22.
