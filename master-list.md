...
### S20b — AST-merge PRODUCTIZED: §4.4-v3 reaches `giti merge` (2026-07-19)

The design fork deferred twice (S18, S19) was opened and resolved: **productize with a JS driver**. Rationale accepted by the operator — giti's jj engine is *already* JS for the identical reason (no subprocess primitive), so a JS merge driver lands in the same already-accepted exception bucket rather than opening a new one, and gets swapped to scrml when the primitive arrives. Slice-4 was first re-verified on the current compiler (`1c577da5`): CLEAN accepts, DANGLING catches `E-TYPE-063`, git control ships the break silently — the measured dominance still holds.

**Slice 1 — engine conflict primitives** (`cacaf8f`). `fileAt(rev,path)` (verbatim), `mergeBase(a,b)` via `heads(::a & ::b)`, `parents(rev)`; `conflicts()` promoted into `EngineInterface`. Design: whole-file sides are fetched **by revision**, never by parsing jj's materialized conflict markers — those are a human-facing display format (jj's own diff-style hunks, *not* git diff3) and the entity merger needs three whole parseable files. Two bugs found:
- **`conflicts()` never reported filenames on real jj.** It parsed `jj status` for a `C <path>` prefix; jj 0.41 lists conflicted paths under a `Warning: There are unresolved conflicts at these paths:` header with no status letter. `files` was **always `[]`** while `hasConflicts` stayed true via the message regex — giti could tell THAT a conflict existed, never WHICH file. Invisible to the suite because the unit mock encoded the same `C ` fiction (**mock drift**) and the integration test only covered the clean case. Now uses `jj resolve --list`.
- **`friendlyError()` inverted its own meaning.** `/conflict/i` matched jj's CLEAN-tree message "No conflicts found at this revision" and rewrote it to "Merge conflict detected." Guarded. Architectural fix: `run()` gained `rawError` — friendlyError is a *presentation* concern, so error *classification* now reads jj's original text. Classifying on prose is what let this hide.

**Slice 2 — driver into product code** (`1f6c926`). `src/merge/scrml-merge.js`: content-in/verdict-out, giti's `ok`/`err` tuples, giti's own `resolveCompilerPath()`. A `SEMANTIC_CONFLICT` result deliberately carries **no `merged` text**, so a caller that forgets to check the verdict crashes rather than writing a type-broken file. Prototype retained in `docs/` as the research record.

**Slice 3 — wired into `giti merge`** (`dfe7b66`). Two deliberately separate passes: `autoResolveConflicts` (entity merge + gate, on **textual conflicts**) and `validateMergeResult` (**the gate alone, on every merged `.scrml`**). §4.3.4 SHALL satisfied: every outcome logged to `.giti/merge-log.json`, reviewable via **`giti status --merge-log`** (new flag).

**DESIGN BUG CAUGHT BY RUNTIME VERIFY — the session's methodological lesson.** The first wire-in gated validation on `conflicts().hasConflicts`. Running the real CLI against the slice-4 dangling fixture showed jj **auto-merges it with no conflict at all** (the sides touch disjoint lines) and silently produces the type-broken file — so the conflict-gated design skipped *precisely the class the feature exists to catch*. That is the slice-4 write-up's own thesis ("git ships it silently"), rebuilt as a blind spot one layer up. **All 415 tests passed while the bug was live; only running the binary found it.** Validation is now independent of conflict state, with a regression test whose engine stub reports zero conflicts on purpose. This is the same lesson as S16's "server-200 ≠ renders", in a new domain: *the gate must sit where the failure is silent, not where the tool already flags something.*
