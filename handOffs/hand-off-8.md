...
- **2026-04-25 — incoming `2026-04-25-2330-...sidecars-found-my-mistake.md`** archived to `read/`. `needs: fyi`. scrmlTS confirms sidecars received + read; their previous "no sidecar" claim was an archiving oversight on their side. No action required.

- **2026-04-25 — slice 5 (commit `f694eaf`):** Unblock `giti land`'s compiler gate. Moved `src/types/{branch,change,landing,repository,typed-change}.scrml` → `docs/spec-types/` (spec illustrations using future state-machine syntax that current scrml doesn't accept). Added path-prefix exclusions in `findScrmlFiles` (`docs/`, `node_modules/`, `dist/`). Side fix: `runTestsDefault` was reading `bun test`'s pass count from stdout, but bun writes the summary to stderr — the dashboard's tests gate showed `count: ?`. Fixed; dashboard now shows `tests: pass — 310` live. 3 new tests for findScrmlFiles exclusions.

- **2026-04-25 — slice 6 (commit `976ee96`):** Remove GITI-011 workaround. Switched all 5 UI pages from HTML `<link>` injection (compileUi post-process) to native `@import url('theme.css')` in each page's `#{}` block. `injectSharedCss` → `copySharedCss` (file-copy only). 9 tests cover copy rules; idempotency, gating on matching `.scrml` stem, sorted output, mixed shared+per-page, missing dirs. Live HTTP smoke confirms zero `<link>` injections + every emitted CSS starts with `@import`. Net -2 tests (the inject-specific tests are gone).

- **2026-04-25 — slice 7 (commit `fb6ee1b`):** Private scopes 6.1 — new `giti private check <pattern>` dry-run subcommand. Lists tracked files matching <pattern> without modifying the manifest. New engine method `files()` wrapping `jj file list`. Detects "already in manifest" vs new-pattern paths and prints contextual advice. Live tested: 14 matches for `ui/**/*.scrml`. 8 new tests in tests/private.test.js.

## End-of-S8 state

- **316 tests pass / 0 fail**
- **8 commits ahead of origin/main** (push request sent to master at 0750)
- Dashboard preflight against the real repo: **all four gates green** (private/conflicts/compiler/tests, 14 .scrml files, 316 tests)
- Inbox empty
- Working tree clean
