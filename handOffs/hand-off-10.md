...
| GITI-014 | **FIX LANDED upstream (scrmlTS S122 `18b90f12`, wrap SHA `a2eb9096`)** — verify next session after scrmlTS pull | Residual of GITI-013: zero-arg arrow `() => ({...})` lost parens in client-emit. Fixed at 5 thunk emit sites in `emit-logic.ts` (broader than my report). Recompile UI repros after pulling scrmlTS to S122; confirm all 5 pages render real data. Repro: `ui/repros/repro-10-zero-arg-arrow-object-init.scrml`. |
| GITI-015 | OPEN — filed to scrmlTS S10 | `is some` in ternary + computed-member LHS not lowered. Workaround: hoist to const. Repro `repro-11`. |
| GITI-016 | OPEN — filed to scrmlTS S10 | variable name `match` parse confusion (`<match>` keyword conflict). Workaround: rename. Repro `repro-12`. |
| GITI-017 | OPEN — filed to scrmlTS S10 | **SILENT CORRUPTION** — `not` keyword substituted inside regex literals. Workaround: char-class split `n[o]t`. Repro `repro-13`. |
| GITI-018 | OPEN — filed to scrmlTS S10 | only first `scrml:` stdlib import per file rewritten in library mode. Workaround: anchor pattern. Repro `repro-14`. |
...
## SESSION 10 CLOSE SUMMARY (2026-05-24)

**21 slices, 21 commits ahead of origin/main.** 371 pass / 0 fail throughout.

### What shipped
1. **GITI-012/013 verified CLOSED** + workarounds removed (slice 1)
2. **DRIFT-1**: `null`→`not` sweep across 5 UI pages (slice 2)
3. **GAP-6 `giti check`** — dry-run validation, 15th command (slice 4)
4. **GAP-8 `giti history --since`** — time-window filter (slice 5)
5. **jj 0.41 installed + colocated** — engine-bound tests now run (was 15 skip → 0 skip)
6. **scrml-as-logic dogfood: 17 modules, ~865 LOC of scrml** shipping in giti's runtime path (slices 6–21)

### Dogfood scoreboard (final)
17 scrml modules in `src/lib/`: duration, parse-status, scope-match, cli-args, save-message, bookmarks, format-status, friendly-error, save-routing-pure, resolve-compiler, remotes, scope-manifest, save-routing-async, find-scrml-files, server-helpers, classify-from-status, result.

JS files reduced to thin re-export shims: `src/private/scope.js` (240→24 LOC), `src/private/remotes.js` (148→17), `src/private/save-routing.js` (240→~20), `src/commands/status.js`, `src/commands/save.js`, `src/commands/sync.js`, `src/commands/history.js`, `src/commands/land.js`, `src/engine/jj-cli.js`, `src/server/index.js` all partially shimmed.

### 5 compiler bugs filed upstream (all with minimal repros)
| ID | Severity | Status |
|---|---|---|
| GITI-014 | high (UI broken) | **FIXED** scrmlTS S122 `18b90f12` — verify after pull |
| GITI-015 | author-level | open — `is some` ternary + computed LHS |
| GITI-016 | author-level | open — `match` identifier parse confusion |
| GITI-017 | **silent corruption** | open — `not` substituted inside regex literals |
| GITI-018 | material | open — only first `scrml:` import rewritten in library mode |

### Friction catalog (DF-1 … DF-11) — see master-list §G
Mostly ergonomics/discoverability. Key ones:
- DF-6 (downgraded): scrml stdlib is broad — `scrml:path/fs/process/...` cover most of Node. Use `scrml:X` not `node:X`.
- DF-9/GITI-018: multi-stdlib import needs the "anchor pattern".
- DF-10: scrml user source shouldn't use `async` (auto-await for typed Promise<T>); explicit async needed at untyped JS-host boundaries.
- DF-11: `scrml:fs.statSync` returns `{isFile, isDirectory}` as booleans, not methods.

### Library-mode workflow (how to regen a scrml lib module)
```
bun run ../scrmlTS/compiler/src/cli.js compile src/lib/<name>.scrml -o src/lib --mode library
```
Produces `src/lib/<name>.js` (ESM, names preserved). Generated `src/lib/_scrml/` shims (path.js, fs.js, process.js) ride along; `dist/` artifacts are gitignored.

### Inbox at S10 close
- All clear. scrmlTS processed every bug report (012/013, 014, 015, 016, 017, 018 in their read/).
- `2026-04-26 S42-close fyi` — STILL unread in giti incoming (informational, no action; kickstarter v1 + F4 routing-leak). Archive or action next session.
- `2026-05-23-0810 GITI-014-fixed fyi` — read + archived this session.

### S11 priorities (suggested)
1. **Pull scrmlTS to S122, verify GITI-014 fix** — recompile UI repros + `giti serve`, confirm all 5 pages render real data again (the dogfood UI was the original blocker). User-driven pull.
2. **Verify GITI-015/016/017/018** if scrmlTS shipped fixes — recompile the affected scrml libs, drop workarounds where the upstream fix lands.
3. **Continue/finish dogfood** if desired: remaining JS is engine subprocess (`Bun.spawn` — needs vendor: shim or injection), CLI command bodies, runCompiler/runTests.
4. **Possible refactor**: migrate async ports + Result tuples to scrml's `safeCallAsync` + `!{ }` values-not-exceptions idiom (DF-10 follow-up).
5. **Theme dedupe** in status.scrml (long-pending).
6. **License selection** (still pending — user S9: "as long as i can charge").
7. **Auth + multi-repo** (master-list §E — blocks hosted forge).

### Pending decisions
- **Push**: S10's 21 commits pushed via master coordination at close (this session). License + S42-fyi archival carried forward.
